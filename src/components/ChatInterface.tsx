...
export const ChatInterface = ({ onClose }: ChatInterfaceProps) => {
  ...
  // ✨ Send message handler with Gemini + fallback simulation
  const handleSendMessage = async () => {
    if (!inputValue.trim()) return;
    ...
    setMessages((prev) => [...prev, userMessage]);
    const currentInput = inputValue; // keep backup
    setInputValue("");
    setIsTyping(true);
    ...
    try {
      ...
      toast({
        title: "Fallback Mode",
        description: "Gemini API failed, showing simulated AI response.",
        variant: "destructive",
      });
    } finally {
      setIsTyping(false);
